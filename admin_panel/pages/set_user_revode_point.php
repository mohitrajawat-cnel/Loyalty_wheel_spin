...
if(isset($_POST["reward_point_save"]) )
{

  $user_reward_get_points    =   json_encode($_POST['user_reward_get_points']);
    ...
    $update ="UPDATE ".$table_prefix."user_redeem_spin_points SET
    user_reward_get_points = '".$user_reward_get_points."'
    WHERE id='1'
    "; 
    ...
    {
      $insert ="INSERT into ".$table_prefix."user_redeem_spin_points SET
      user_reward_get_points = '".$user_reward_get_points."'
              ...
              <div class="bg-gradient-primary shadow-primary border-radius-lg pt-2 pb-1">
                <div class="row">
                  <div class="col-md-12 pt-2">
                    <h6 class="text-white text-capitalize ps-3">Add User Reward Point</h6>
                  </div>
                </div>
              </div>
            </div>
              ...
              <form class="form_data" method="post">
                <div class="row">
                 ...
                 <div class="form-group col-md-6">
                    ...
                    ?>
                      <div id="get_all_rewards_value" class="form-control" style="border: 1px solid lightgray;">
                             <option value="" >Set User Reward get Points</option>
                             <?php
                            //ramkishan
                             $select="SELECT * FROM ".$table_prefix. "user_redeem_spin_points";
                             $row=$conn->query($select);
                             $result_row=mysqli_fetch_assoc($row);

                             $data_show =  json_decode($result_row['user_reward_get_points'],true);
                                 ...
                                 while($result_reward = mysqli_fetch_assoc($row_reward))
                                 {
                                    ...
                                    {
                                        $showing_reward_list = str_replace('<br />','',urldecode($all_data_show['prize'.$i]));
                                       
                                        ?>
                                        <div>
                                            <input name="<?php echo $i; ?>" value="<?php echo $showing_reward_list; ?>" style="border-radius: 5px;margin: 5px 5px 0px 0px;" readonly>
                                            <input name="user_reward_get_points[<?php echo $i; ?>]" class="reward_value_for_reamin_spin" style="border-radius: 5px;" value="<?php echo $data_show[$i] ? $data_show[$i] : 0;  ?>">
                                        </div>  
                                        <?php
                                    }
                                 }
                ...
                <div class="form-group mt-4">
                  <button type="submit" name="reward_point_save" class="btn btn-primary">Submit</button>
                </div>
