<?php
    ...
    if(isset($_POST['game_entered_code']))
    {
        $error='';
        $output = array();
        $code = "";
        ...
        $code =	$_POST['game_entered_code'];
        $select="SELECT * FROM `".$prefix."plinko_game_code_data` WHERE BINARY `code` = '$code'";
        ...
        if(mysqli_num_rows($query)==0)
        {
            $code_id = null;
            $csshwe = "display: none";
            $error='<span id="login-password-error" class="error" style="color:red;">Please Enter Correct Code.</span>';
        }
        else
        {
            while($row=mysqli_fetch_array($query))
            {
                ...
                else
                {
                    $code_id = null;
                    $csshwe = "display: none";
                    $error='<span id="limit_exceed" class="error" style="color: red;">Number of attempt exceed.</span>';
                }
            }
        }
        $output = ["code" => $code, "code_id" => $code_id, "error" => $error];
...
<html style="background: black;">
    ...
    <body ondragstart="return false;" ondrop="return false;">
        <input type="hidden" name="get_data_session" value="" id="data_of_code">
        <div id="selectHidden_field_val">
            <?php
            ...
            echo '<input type="hidden" name="" value="'.$game_withCode_orNot.'" id="game_withCode_orNot">';
        ...
        <div class="modal" id="loginModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" >
            ...
            <div class="modal-content">
                <form method="post" action="" id="login_with_code_hwe">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-6 custom_add_margin_user">
                            <input type="text" name="game_entered_code" class="form-control design_password" id="code" placeholder="Enter your code">
                        </div>
                        </div>
                    </div>
        ...
        <script>
            $(document).ready(function(){
                ...
                $("form#login_with_code_hwe").on('submit', function(e){
                    // alert("hello");
                    e.preventDefault();
                    $.ajax({
                        url: "",
                        ...
                        success: function(response){
                            if (response.error === "") {
                                $("#loginModal").hide();
                                $("#data_of_code").val(response.code);
                            } else {
                                // console.log(response.error);
                                $("form#login_with_code_hwe .erroe_msg").html(response.error);
                                $("#selectHidden_field_val").css("display", "none");
                            }
                        }
                    });
                });

                if(jQuery("input[type='hidden']#game_withCode_orNot").val() === "1") {
