...
///login with name&email&mobile_number
if(isset($_POST['usern_e_m_save']))
{
    $username_hwe = $_POST['username'];
    $useremail_hwe = $_POST['useremail'];
    $usermobilenumber_hwe = $_POST['usermobilenumber'];

 //ramkishan
   $select="select * from ".$table_prefix."codegenerate where (user_n_e_mob_email='$useremail_hwe' || user_n_e_mob_phnumber='$usermobilenumber_hwe') && status='1'";
    ...
    {
        $error_code='<span id="login-password-error" class="error" style="color:red;">Details already exists or incorrect.</span>';
        ?>
            <script>
    ...
    {
        $select8="select * from ".$table_prefix."codegenerate where (user_n_e_mob_email='$useremail_hwe' || user_n_e_mob_phnumber='$usermobilenumber_hwe') && status='0'";
   //ramkishan
        ...
        if(mysqli_num_rows($query8) > 0)
        {
            ...
            $update_limit ="UPDATE ".$table_prefix."codegenerate SET `status` = '1',created=now()"; 
    ...
    //ramkishan
        ...
        {
            //ramkishan
            $update_limit ="INSERT into ".$table_prefix."codegenerate SET user_n_e_mob_username = '$username_hwe',
            user_n_e_mob_email = '$useremail_hwe',
            user_n_e_mob_phnumber = '$usermobilenumber_hwe',
            `status`='1',
            method='n_e_m',
            created=now()"; 
            mysqli_query($conn,$update_limit);
        ...
        ?>
         <script>
            var get_reamin_spin_code1 =0;
             get_reamin_spin_code1 = '<?php echo $get_remain_spin; ?>';
        </script>
        <script>
            var spain_code1 = '<?php echo $_SESSION['spain_code']; ?>';
        </script>
        <script>
            jQuery(document).ready(function(){
                    jQuery("#drawing").show();
                    ...
                    setTimeout(function () {
                       ...
                       <?php 
                            if($code_sp == '1' || $mobile_number_sp == '1' || $code_with_remain_spin_sp == '1' || $name_email_mobileno == '1')
                            {
                                ?>
